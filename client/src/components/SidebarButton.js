...
function SidebarButton({ className, selected, onClick, iconSrc, onIconClick, iconRotated, handleImageError, text, editable, subscription }) {
    ...
    return (
        <div className={`${className ? className : ''} ${styles['sidebar-btn']} ${selected ? styles['selected'] : ''}`} onClick={onClick}>
            {iconSrc!== undefined && <img 
                className={`${styles['btn-icon']} ${iconRotated ? styles['icon-rotated'] : ''}`} 
